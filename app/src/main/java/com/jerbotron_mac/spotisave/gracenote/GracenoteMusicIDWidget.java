...
public class GracenoteMusicIDWidget {
	...
	public GracenoteMusicIDWidget(Context in_context) {
		try{
	...
	public void GnInit(){
		...
		if ( (GNSDK_LICENSE_FILENAME == null) || (GNSDK_LICENSE_FILENAME.length() == 0) ){
			...
			if ( gnsdkLicense == null ){
				setStatus( "License file not found: " + GNSDK_LICENSE_FILENAME);
				return;
			}
		}
		...
		try {
			...
			gnUser = new GnUser( new GnUserStore(context), GNSDK_CLIENT_ID, GNSDK_CLIENT_TAG, appString );
