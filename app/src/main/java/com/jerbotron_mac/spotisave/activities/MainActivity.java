package com.jerbotron_mac.spotisave.activities;
...
import com.jerbotron_mac.spotisave.ui.DetailView;
...
public class MainActivity extends Activity {
    ...
    public static final String    		gnsdkLogFilename 		= "sample.log";
    ...
    public  ViewGroup metadataListing;
    ...
    public volatile boolean 			lastLookup_local		 = false;	// indicates whether the match came from local storage
    public volatile long				lastLookup_matchTime 	 = 0;  		// total lookup time for query
    public volatile long				lastLookup_startTime;  				// resume time of query
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if ( (GNSDK_CLIENT_ID == null) || (GNSDK_CLIENT_TAG == null) ){
        ...
        if ( (GNSDK_LICENSE_FILENAME == null) || (GNSDK_LICENSE_FILENAME.length() == 0) ){
            showError( "License filename not set" );
        } else {
            gnsdkLicense = getAssetAsString(GNSDK_LICENSE_FILENAME);
            if ( gnsdkLicense == null ){
                showError( "License file not found: " + GNSDK_LICENSE_FILENAME);
                return;
            }
        }
        ...
        try {
            ...
            gnUser = new GnUser( new GnUserStore(context), GNSDK_CLIENT_ID, GNSDK_CLIENT_TAG, APP_STRING );
    ...
    private class MusicIDFileEvents implements IGnMusicIdFileEvents {
        ...
        public void gatherFingerprint(GnMusicIdFileInfo fileInfo, long currentFile, long totalFiles, IGnCancellable cancelable){
            ...
            try {
                ...
                {
                    Log.e(APP_STRING, "error in fingerprinting file: " + e.errorAPI() + ", " + e.errorModule() + ", " + e.errorDescription());
                }
            }
        }
        ...
        public void musicIdFileStatusEvent(GnMusicIdFileInfo fileinfo, GnMusicIdFileCallbackStatus midf_status, long currentFile, long totalFiles, IGnCancellable canceller){
            ...
            try {
                ...
                Log.e(APP_STRING, "error in retrieving musidIdFileStatus");
            }
        ...
        public void musicIdFileResultNotFound( GnMusicIdFileInfo fileInfo, long currentFile, long totalFiles, IGnCancellable cancellable ) {
            ...
            try {
                Log.i(APP_STRING,"GnMusicIdFile no match found for " + fileInfo.identifier());
            } catch (GnException e) {
            }
        }
        ...
        public void musicIdFileComplete( GnError musicidfileCompleteError ) {
            ...
            if ( musicidfileCompleteError.errorCode() == 0 ){
                ...
                Log.e(APP_STRING, musicidfileCompleteError.errorAPI() + ": " + musicidfileCompleteError.errorDescription() );
            }
            setUIState( UIState.READY );
        }
    ...
    private String getAssetAsString( String assetName ){
        ...
        try {
            ...
            if(assetStream != null){
                ...
                Log.e(APP_STRING, "Asset not found:" + assetName);
            }

        } catch (IOException e) {

            Log.e( APP_STRING, "Error getting asset as string: " + e.getMessage() );
